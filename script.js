...
var snake_tail = []; // snake tail array
var snake_total = 0; // number of tail segments
var scoreVal = 0; // score

function setup() {
  createCanvas(800, 500); // adjusted canvas to match food random range
  snake_x = width / 2;
  snake_y = height / 2;
  snake_xspeed = 1;
  snake_yspeed = 0;
...
function spawnFood() {
  var foodXPos = floor(random(width / scl)) * scl;
  var foodYPos = floor(random(height / scl)) * scl;
  food = [foodXPos, foodYPos];
}

function keyPressed() {
  if (keyCode === UP_ARROW && snake_yspeed !== 1) {
    snake_xspeed = 0;
    snake_yspeed = -1;
  } else if (keyCode === DOWN_ARROW && snake_yspeed !== -1) {
    snake_xspeed = 0;
    snake_yspeed = 1;
  } else if (keyCode === RIGHT_ARROW && snake_xspeed !== -1) {
    snake_xspeed = 1;
    snake_yspeed = 0;
  } else if (keyCode === LEFT_ARROW && snake_xspeed !== 1) {
    snake_xspeed = -1;
    snake_yspeed = 0;
  }
}
